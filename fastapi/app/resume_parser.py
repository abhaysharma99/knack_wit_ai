...
def extract_text_from_pdf(file_bytes: bytes) -> str:
    ...
    Preserves layout better than pypdf for detecting resume sections.
    """
    try:
...
def _strip_fences(raw: str) -> str:
    """
    Clean Gemini response to get valid JSON string.
    Handles:
    - markdown fences: ```json ... ```
    - missing outer braces (Gemini starts with newline then "name": ...)
    - leading/trailing whitespace and preamble text
    """
    cleaned = raw.strip()
    ...
    # Find the first { — handles preamble text before JSON
    brace_idx = cleaned.find("{")
    if brace_idx != -1:
        cleaned = cleaned[brace_idx:]
    else:
        # No { found at all — wrap the whole content
        cleaned = "{" + cleaned + "}"
...
def parse_resume(resume_text: str) -> dict:
    """
    Parse raw resume text to structured dict using Gemini.
    Returns None on failure so the pipeline continues without crashing.
    """
    if not resume_text or not resume_text.strip():
        logger.warning("Empty resume text — skipping parse.")
        return None
    ...
    for attempt in range(2):
        try:
            ...
            last_error = e
            logger.warning("Resume parse attempt %d failed: %s — raw: %s", attempt + 1, e, raw[:100] if 'raw' in dir() else 'N/A')
        except Exception as e:
            last_error = e
            logger.exception("Unexpected error during resume parsing: %s", e)
            break

    logger.error("Resume parsing failed after retries: %s", last_error)
    return None
