...
class GrowthFeatures(BaseModel):
    """The 4 computed growth signals for a candidate."""
    growth_velocity:   float  # how fast they leveled up vs years of experience
    consistency:       float  # how focused/specialized their skill set is
    expansion_rate:    float  # breadth across skill categories
    complexity_growth: float  # project count x avg technologies per project


class AnalysisResult(BaseModel):
    """Full breakdown returned by GET /candidate/{candidate_id}/analysis."""
    candidate_id:      str
    candidate_name:    Optional[str]   = None
    domain:            Optional[str]   = None
    seniority:         Optional[str]   = None

    # Phase 2 scores (current fit)
    current_fit_score: Optional[float] = None

    # Phase 3 scores
    growth_features:    GrowthFeatures
    future_potential:   float          # XGBoost output (0-1)
    true_talent_score:  float          # 0.6 x current_fit + 0.4 x future_potential
    hidden_genius_flag: bool           # True if future_potential > 0.65

    # Explainability
    reasons:   List[str]               # why this score
    strengths: List[str]               # what candidate is strong at
