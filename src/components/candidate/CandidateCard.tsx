...
export default function CandidateCard({ candidate, index }: CandidateCardProps) {
  const { setSelectedCandidate, setShowAnalysisModal } = useAppStore()
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white/40 text-sm mb-2">{candidate.college} • {candidate.experience_years} years</p>
          ...
          <div className="grid grid-cols-3 gap-2">
            <ScoreBadge 
              icon={Star} 
              label="Current Fit" 
              score={candidate.current_fit_score} 
              color="text-talent-400" 
            />
            <ScoreBadge 
              icon={TrendingUp} 
              label="Future Potential" 
              score={candidate.future_potential_score} 
              color="text-genius-400" 
            />
            <ScoreBadge 
              icon={Award} 
              label="True Talent" 
              score={candidate.true_talent_score} 
              color="text-success" 
              highlight
            />
          </div>
