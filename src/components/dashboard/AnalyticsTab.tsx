            ...
            {topSkills.map((skill, index) => (
              ...
              >
                ...
                <div className="flex-1 h-3 rounded-full bg-white/10 overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${(skill.count / 487) * 100}%` }}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-talent-500/10 border border-talent-500/20">
              <div className="text-3xl font-bold text-talent-400 mb-1">
                {Math.round(candidates.filter(c => c.hidden_genius).reduce((acc, c) => acc + c.future_potential_score, 0) / candidates.filter(c => c.hidden_genius).length)}
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-talent-400 font-bold">
                {Math.round(dashboardStats.total_candidates / (dashboardStats.processing_time_ms / 1000))}
              </span>
            </div>
          </div>
        </GlowingCard>
